 ...
 * @author js
 */
public class speedSign2 extends trackObject {

	static int sc = 0;
	static Color col_hgr = new Color(0xff, 0xff, 0xff);
	...
	private void paintSig(Graphics2D g2, int trackDist) {
		...
		g2.fillRect(trackDist + (fscale - fscale / 4) - 1, fscale, 2, fscale * 3);
		g2.setColor(col_hgr);
		Polygon p = new Polygon();
		p.addPoint(trackDist, 0);
		p.addPoint(trackDist + fscale * 2 - fscale / 2, 0);
		p.addPoint(trackDist + fscale * 2 - fscale / 2, fscale * 2);
		p.addPoint(trackDist, fscale * 2);
		g2.fillPolygon(p);
		...
		g2.drawString(text, trackDist + (fscale * 2 - fscale / 2 - adv) / 2 - 1, bl + 3);
	}
